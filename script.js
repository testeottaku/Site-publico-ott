...
function handleRoute(path) {
  ...
  // Se a rota estiver no mapa, use-a
  if (ROUTES[path]) {
    pageId = ROUTES[path];
  } else if (path.startsWith('/novidades')) {
    // Se for uma rota de novidades com categoria
    pageId = 'novidades';
  } else {
    // Rota não encontrada, vai para home
    pageId = 'home';
  }
...
function showPage(pageId, category = null) {
  // Primeiro, garante que NENHUMA página esteja visível
...
function filterByCategory(category, updateUrl = true) {
  ...
  if (updateUrl) {
    ...
    currentRoute = path;
    updateMetaTags('novidades', category);
  }
}

// Função para lidar com pesquisa - atualiza URL automaticamente
function handleSearch() {
  ...
  searchTimeout = setTimeout(() => {
    ...
    // Aplica filtro
    filterNews();
  }, 300); // Delay de 300ms para não sobrecarregar o histórico
}
...
async function renderAllNews() {
  ...
  // Aplica categoria se houver
  if (currentCategory !== 'todos') {
    const btn = document.querySelector(`#novidades .category-btn[data-category="${currentCategory}"]`);
    if (btn) {
      document.querySelectorAll('#novidades .category-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
    }
  }
}
