...
class SPARouter {
    constructor() {
        this.routes = {
            ...
            '/gaming': {
                id: 'gaming',
                title: 'Gaming | Ottaku Brasil',
                init: renderGamingCards
            },
            '/parceiros': {
                id: 'parceiros',
                title: 'Parceiros | Ottaku Brasil',
                init: renderPartners
            },
            '/quiz': {
                id: 'quiz',
                title: 'Quiz | Ottaku Brasil',
                init: renderQuizWinnersHome
            },
            '/ganhadores': {
                id: 'ganhadores',
                title: 'Ganhadores | Ottaku Brasil',
                init: () => {
                    filterWinners('todos');
    ...
    executeRoute(routeConfig, params = {}) {
        ...
        if (targetPage) {
            ...
            this.updateActiveNav(routeConfig.id);
            ...
            if (typeof routeConfig.init === 'function') {
                try {
                    ...
                    console.error(`Error initializing page ${routeConfig.id}:`, error);
                }
            }
        }
        ...
        this.updateMetaTags(routeConfig);
    }
    ...
    updateActiveNav(pageId) {
        ...
        const activeLinks = document.querySelectorAll(`[data-page="${pageId}"], a[href="${this.currentPath}"]`);
    ...
    updateMetaTags(routeConfig) {
        ...
        canonicalLink.href = window.location.origin + this.currentPath;
        ...
        ogUrl.setAttribute('content', window.location.origin + this.currentPath);
    }
...
function filterByCategory(category) {
    ...
    // Update URL if needed (but don't push to history to avoid double entries)
    if (category === 'todos') {
        if (window.location.pathname !== '/novidades') {
            window.history.replaceState({}, '', '/novidades');
        }
    } else {
        if (window.location.pathname !== `/novidades/${category}`) {
            window.history.replaceState({}, '', `/novidades/${category}`);
        }
    }
}
...
function filterWinners(category) {
    ...
    if (category === 'todos') {
        if (window.location.pathname !== '/ganhadores') {
            window.history.replaceState({}, '', '/ganhadores');
        }
    } else {
        if (window.location.pathname !== `/ganhadores/${category}`) {
            window.history.replaceState({}, '', `/ganhadores/${category}`);
        }
    }
}
...
function setupGlobalListeners() {
    ...
    // Direct navigation buttons (for buttons that aren't <a> tags)
    document.querySelectorAll('[data-navigate]').forEach(button => {
        button.addEventListener('click', function() {
            const path = this.getAttribute('data-navigate');
            if (path) {
...
// Make router available globally for direct navigation if needed
