          ...
          <div class="card-container">
            <div class="flex items-center gap-4">
              <div class="relative">
                ...
                <div class="badge"><div class="shimmer" style="width:12px;height:12px;border-radius:3px;"></div></div>
              </div>
              <div class="content">
                <div class="header">
                  <div class="shimmer" style="height:16px;width:55%;border-radius:6px;"></div>
                  <div class="shimmer" style="height:16px;width:22%;border-radius:6px;"></div>
                </div>
                <div class="footer">
                  <div class="tags">
                    <div class="shimmer" style="height:16px;width:64px;border-radius:999px;"></div>
                    <div class="shimmer" style="height:14px;width:62px;border-radius:6px;"></div>
                  </div>
                  <div class="shimmer" style="height:28px;width:82px;border-radius:999px;"></div>
                </div>
              </div>
            </div>
          </div>
...
function formatWinnerDate(item){
  // Aceita strings prontas (ex: "Nov 2025")
  const raw = item.dateLabel || item.date || item.monthYear || item.month || item.createdAtLabel;
  if (raw && typeof raw === 'string') return escapeHtml(raw);

  // Firestore Timestamp (seconds/nanoseconds) ou Date
  const ts = item.createdAt || item.created_at || item.timestamp || null;
  let d = null;
  if (ts instanceof Date) {
    d = ts;
  } else if (ts && typeof ts === 'object') {
    if (typeof ts.seconds === 'number') d = new Date(ts.seconds * 1000);
  ...
  // pt-BR, mês abreviado + ano (ex: "fev. 2026") -> "Fev 2026"
...
function getWinnerIconClass(cat){
  const c = (cat || '').toLowerCase();
  if (c === 'instagram') return 'fa-brands fa-instagram';
  if (c === 'gaming') return 'fa-solid fa-gamepad';
  if (c === 'quiz') return 'fa-solid fa-trophy';
...
function renderGeneralWinners(list){
  ...
  grid.innerHTML = list.map(item => {
    ...
    const handleRaw = item.instagramHandle || item.handle || "";
    const handle = escapeHtml(handleRaw ? (String(handleRaw).startsWith('@') ? handleRaw : `@${handleRaw}`) : "");
    const username = handle || name || "@usuario";
    const prize = escapeHtml(item.prize || item.value || item.amount || "");
    const img = escapeHtml(item.imageUrl || item.avatarUrl || item.photoUrl || "");
    const profileUrl = escapeHtml(item.profileUrl || item.url || "");
    const dateLabel = formatWinnerDate(item);

    const linkOpen = profileUrl ? `href="${profileUrl}" target="_blank" rel="noopener"` : `href="#" onclick="return false;"`;
    const badgeIcon = getWinnerIconClass(cat);
    // Ícone do botão "Ver" deve ser sempre Instagram (ação)
    const verIcon = 'fa-brands fa-instagram';
    ...
    return `
      <div class="winner-card-item" data-category="${cat}">
        <div class="card-container">
          <div class="flex items-center gap-4">
            <div class="relative">
              ${img ? `<img src="${img}" alt="${username}" class="avatar">` : `<div class="avatar avatar--placeholder"></div>`}
            ...
            <div class="content">
              <div class="info">
                ...
                <div class="tags">
                  <span class="category">${categoryTitle(cat)}</span>
                  ${dateLabel ? `<span class="date">${dateLabel}</span>` : ``}
                </div>
              </div>

              <div class="right">
                ${prize ? `<span class="price">${prize}</span>` : ``}
                <a ${linkOpen} class="ver-btn" aria-label="Ver no Instagram">
                  <i class="${verIcon}"></i>
                  Ver
                </a>
              </div>
